...
namespace MultiShop.WebUI.Areas.Admin.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMeassage = await client.GetAsync("https://localhost:7070/api/Brands");//get isteği atarız ve bir nesne (paket) döner.
        ...
        [HttpPost]
        [Route("CreateBrand")]
        public async Task<IActionResult> CreateBrand(CreateBrandDto _createBrandDto)
        {
            var client = _httpClientFactory.CreateClient();
            string jsonData = JsonConvert.SerializeObject(_createBrandDto);
        ...
        [Route("UpdateBrand/{id}")]
        [HttpPost]
        public async Task<IActionResult> UpdateBrand(UpdateBrandDto _updateBrandDto)
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(_updateBrandDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
